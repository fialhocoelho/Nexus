...
class TeacherModel():
    def __init__(self,
        ...
        # Define paths
        self.raw_dir = os.path.join(to_root_dir, self.data_params['raw_path'])
        ...
        train_folder_path = os.path.join(self.raw_dir,
                                        self.data_params['train_folder'])

        test_folder_path = os.path.join(self.raw_dir,
                                        self.data_params['test_folder'])

        df_train = pd.read_parquet(os.path.join(train_folder_path,
                                                'waves_palmas.parquet'))
        df_test = pd.read_parquet(os.path.join(test_folder_path,
                                                'waves_palmas.parquet'))
        ...
        train_start_date = self.data_params['train_start_date']
        train_end_date = self.data_params['train_end_date']
        test_start_date = self.data_params['test_start_date']
        test_end_date = self.data_params['test_end_date']
        freq = self.data_params['target_freq']  # Frequência de 1 hora
        interp_method = self.data_params['interp_method']
        datetime_col= self.data_params['datetime_col']
        round_freq= self.data_params['round_freq']
        ...
        df_test_processed = process_dataframe(df_train,
                                                start_date,
                                                end_date,
                                                freq,
                                                interp_method,
